...
Personal agents never act as the user on GitHub. The mechanism is not a policy in the shims but a container that simply lacks the user's credentials: `scripts/agentbox` starts one box per repository from this image under the Sysbox runtime (`sysbox-runc`), mounts an allow-list of the home directory (the repo and its `~/.worktrees/<repo>` workspaces, `~/.dotfiles`, mise's store, omp state, jj and gh-app routing config, the secretsd socket, the `core-context` and `core-ops` skill farms) and nothing else, and on a host marked by `installers/agentbox.sh` (run once after Sysbox is installed; `disable` unmarks) `shims/omp` refuses to start outside a box. Unmarked machines run omp directly.
