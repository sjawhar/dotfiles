...
- **Showing Sami devbox content:** link files as `http://localhost:12802/<abs-path>` (get one with `forward url <path>`) instead of `file:///` links, which the laptop resolves against the wrong filesystem. A web app or any other TCP port, on the devbox or inside an agent box, reaches his laptop's `localhost:<port>` while `forward port <ports>…` runs; the `forward-port` skill has the supervised invocation and its failure modes.
