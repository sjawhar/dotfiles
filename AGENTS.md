...
- **Agent boxes are opt-in.** `scripts/agentbox` runs one Sysbox container per agent session from `devpod/Dockerfile`, working in a jj workspace made for it: `agentbox omp <repo> [omp args…]` names a canonical checkout under `~/src`, adds a workspace of it at `~/boxes/<box>/<repo>`, runs omp there as the container's main process, and at exit releases the session's knives claims, snapshots, forgets the workspace and removes the directory — work survives as commits in the canonical repo. The box mounts an explicit allow-list of the home directory (checkouts, omp state, dotfiles, mise store, `~/.aws`, `~/.kube`, `~/.pulumi`, tool configs, the jj-signing pair, the host sockets) and nothing else: no GitHub login, ssh keys, keyrings or docker login, so a session pushes only as its repo's GitHub App and can clutter only the repos and its own deleted box dir. AWS is deliberately different: every profile works as on the host through a metadata-service forwarder on the box network (`AWS_EC2_METADATA_SERVICE_ENDPOINT`), while containers the box starts are firewalled off it inside the box. Hawk tokens (`!hawk-token`, the anthropic provider) and the Envoy HTTP API are host-forwarded the same way (`agentbox-hawk-token`, `agentbox-envoy`), so the keyring never enters the box. `omp` on the host is unchanged. tmux-resurrect resumes a boxed pane boxed (`agentbox omp <repo> --resume <id>`) and a host pane on the host (`scripts/tmux-resurrect-omp`). `agentbox doctor` health-checks both sides. Canonical checkouts: `~/src/<repo>` (fresh clones; `~/src/dotfiles` is a symlink to the installed `~/.dotfiles`).
