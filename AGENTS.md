...
- **Agent boxes are opt-in.** `scripts/agentbox` runs one Sysbox container per repository from `devpod/Dockerfile` with an allow-list of home-directory mounts and no user credentials, so a session started with `agentbox omp <repo>` can push only as its repo's GitHub App. `omp` on the host is unchanged. tmux-resurrect resumes a boxed pane boxed (`agentbox omp <repo> --resume <id>`) and a host pane on the host (`scripts/tmux-resurrect-omp`).
