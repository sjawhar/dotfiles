...
- **Agent boxes are opt-in.** `scripts/agentbox` runs one Sysbox container per agent session from `devpod/Dockerfile`: `agentbox omp [DIR] [omp args…]` mounts the whole home directory minus a hidden list of the user's personal third-party logins (each shadowed by an empty per-box dir; the jj-signing pair is re-exposed), so the session behaves like a host session but pushes only as its repo's GitHub App. AWS is deliberately different: `~/.aws` is visible and the box reaches the EC2 metadata service through a host forwarder on the box network (`AWS_EC2_METADATA_SERVICE_ENDPOINT`), so every AWS profile works as on the host while containers the box starts are firewalled off it inside the box. Hawk tokens (`!hawk-token`, the anthropic provider) are minted on the host too: a second transient unit (`agentbox-hawk-token`) serves `scripts/hawk-token` over a socket the box mounts, so the keyring that holds the Hawk session never enters the box. The inner dockerd starts lazily on the first `docker` call; its image store is a per-session volume reattached on `--resume`/`-c` and reaped by `agentbox gc`. Workspaces a boxed session creates (`jj workspace add`, `knives start`) are cleaned up when the box exits and recreated on `--resume`. `omp` on the host is unchanged. tmux-resurrect resumes a boxed pane boxed (`agentbox omp --resume <id>`, in the saved directory) and a host pane on the host (`scripts/tmux-resurrect-omp`). `agentbox doctor` health-checks both sides.
