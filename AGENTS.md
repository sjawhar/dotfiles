...
- **Agents run in boxes.** On a host marked by `installers/agentbox.sh` (a machine-local role installer, run once where Sysbox is installed), `shims/omp` refuses to start outside a box and names `agentbox omp <repo>`; unmarked machines (the laptop) run omp directly. `scripts/agentbox` runs one Sysbox container per repository from `devpod/Dockerfile` with an allow-list of home-directory mounts and no user credentials, so an agent can push only as its repo's GitHub App. `AGENTBOX_BYPASS=1 omp …` runs on the host for a session with no repo to box or for host-privileged work. tmux-resurrect saves each omp pane as `agentbox omp <repo> --resume <id>` (`scripts/tmux-resurrect-omp`), so a restore resumes sessions inside their boxes.
