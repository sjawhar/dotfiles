...
class GwsRouting(unittest.TestCase):
    ...
    def test_non_ec2_passes_through_to_user_credentials(self):
        # Personal machines (laptops) have no instance role/IMDS: the broker
        # cannot work there. gws must run untouched so ~/.config/gws user
        # credentials (and `gws auth login`) keep working.
    ...
    def test_missing_dmi_file_passes_through(self):
        # No DMI sysfs at all (macOS, some containers) -> not EC2 -> pass through.
        write_stub(self.stub_dir, "google-user-token", "echo SHOULD-NOT-RUN >&2; exit 1")
        result = self.run_shim({"GWS_SHIM_DMI_SYS_VENDOR": str(self.stub_dir / "does-not-exist")})
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn("TOKEN=\n", result.stdout)
        self.assertNotIn("SHOULD-NOT-RUN", result.stderr)

    def test_gmail_credentials_work_on_non_ec2(self):
        # The YubiKey-gated Gmail-send path is machine-independent.
        self.dmi_file.write_text("LENOVO\n", encoding="utf-8")
        result = self.run_shim({"GMAIL_OAUTH_CREDENTIALS": '{"type":"authorized_user"}'})
        self.assertEqual(result.returncode, 0, result.stderr)
        self.assertIn('{"type":"authorized_user"}', result.stdout)
