...
- **Tool installers** — `shell.sh`, `mise.sh`, `docker.sh`, `sops.sh`, `jj.sh`, `tmux.sh`, `nvim.sh`, `claude.sh`, `opencode.sh`, plus others (`ghostty.sh`, `voxtype.sh`, `tt.sh`, `whatsapp.sh`, `knives.sh`). `docker.sh` installs Docker Engine from Docker's own repo and puts the login user in the `docker` group — never Ubuntu's `docker.io`, whose `containerd` conflicts with `containerd.io` and which ships no buildx or compose plugin. Group membership is what SSH-driven tooling (Pulumi's docker provider, the envoy deploy) needs, since it gets no tty for sudo.
