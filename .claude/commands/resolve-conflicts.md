...
4. After resolving all conflicts, run `jj status` to confirm no conflicts remain
5. Run the project's quality checks (type checking, linting, tests) to verify the resolution
6. If checks pass, push with `jj git push`
7. The PR will auto-update; report the result
