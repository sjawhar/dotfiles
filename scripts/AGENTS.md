...
- **OpenCode/Claude/omp sessions** — `agentbox` (`omp <repo>|sh|ls|doctor|rebuild`: one Sysbox box per agent session in a fresh jj workspace of `~/src/<repo>`; `omp` inside it is the same `shims/omp`, the box dies with the session, and its workspace and knives claims go with it), `oc`, `oco`, `ompo` (omp under a named profile, billed only to that profile's `*_API_KEY_<NAME>` secrets; see `shims/omp`), `cco`, `cld`, `claude-session-repair.py`, `claude-transcript-times`, `sanitize-opencode-db`, `claudeforge-install`.
