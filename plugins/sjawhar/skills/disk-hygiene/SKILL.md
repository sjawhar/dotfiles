...
**Correction, observed 2026-09-25 on the same store: the damage is NOT always confined to the forgetting session's own slots.** Twice in one night, admin entries for workspaces belonging to *other, live* sessions vanished — the store went 69 → 62 entries across a window whose op log holds exactly one `forget workspace`, by a different session on a different agentbox. So treat any `forget` on a shared store as able to strip any slot, and check every survivor you care about, not just your own.
