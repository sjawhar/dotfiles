...
- **A successful fetch can still leave the needed ref stale.** When `jj git fetch` exits 0 but an expected remote ref did not move, compare the read-only `git ls-remote origin main` result with `jj log -r main@origin`; this is a diagnostic exception to the no-Git-mutations rule. Before `jj workspace update-stale` in a shared-store workspace, record the nonempty working-copy change and confirm the same change remains after the update; a fresh empty change is not a clean recovery. Both checks apply only to these ambiguous states and are inferred from AGENTC-186's 2026-09-16 retro (platform PO, 2026-09-17).
