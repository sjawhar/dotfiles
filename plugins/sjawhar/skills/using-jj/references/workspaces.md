...
**Canonical checkouts live at `~/src/<repo>` and are not a place to work.** `/home/ubuntu/src/agent-c` (and every other canonical checkout other sessions share) is the store's default workspace: a `jj new`, edit, or commit there lands in a working copy every co-tenant resolves through. Make a named workspace and work only there: `jj -R /home/ubuntu/src/<repo> workspace add /home/ubuntu/.worktrees/<repo>/<name> --name <name>`. That path is the only place a new workspace goes — never `/tmp`, never a clone — and a new one is made only for a genuinely disjoint parallel lane, not because a session "wants its own" (Sami, 2026-09-18: "New workspaces should be in ~/.worktrees/{repo}, not /tmp/ or some other dumb location"). Four stray `jj new`s in shared checkouts on 2026-09-17/18 (the platform PO's at 14:37Z, a dispatched subagent's in `/home/ubuntu/legion` ~04:40Z, two on #19336's own lane) each cost another session a repair. Inferred from those incidents (platform PO via the PR queue, 2026-09-18).

**`/home/ubuntu/agent-c` is the retired store (Sami, 2026-09-19, OPS-96: "start a new store in ~/src/agent-c and migrate only what we need").** Its operation log had grown to 189 GB and every command there costs ~4 s. Do not add workspaces to it. A session still working in one of its workspaces moves its lane with `~/.local/state/disk-hygiene/run-2026-09-18/migrate-lane.sh <old-workspace-dir> <new-name>` (pushes your described commit into the new store by sha, adds a workspace at `~/.worktrees/agent-c/<new-name>`), then `cd`s there. Nothing else is carried: bookmarks nobody pushed, old working copies and the old operation log are deleted with the store once its last session ends.
