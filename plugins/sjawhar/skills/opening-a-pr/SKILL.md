...
11. **Open or update the PR — and default to NOT opening a new one.** One line of work gets one PR. Before creating anything, take stock of what this session already has open in this repo (`jj bookmark list`, `gh pr view 2>/dev/null`, `gh pr list --author @me`): new work in the same line of work folds into the existing PR (`jj git push`, then `gh pr edit`); genuinely dependent multi-PR work stacks via the `gh-stack` skill from the first PR and lands as one reviewed unit, never as piecemeal merge-asks. A spray of sibling one-off PRs from one session is a standing-instruction violation Sami corrected on six separate occasions in a single week ("Stop making more PRs. You know my standing instructions are to make a minimal number of PRs"; "now there are 8 new PRs opened in the past 12 hours. This is fucking ridiculous"). If none exists, set the bookmark from the change description (`jj git push --named=<name>=@`) and `gh pr create` with closing keywords for the issues it resolves. Either way the body must carry a `## Verification` section written for a human: which user-facing workflows were exercised end to end, who ran them, and what was observed. Write behavior, not a command transcript. **Test-suite and static-checker output — `pytest`, `basedpyright`, lint, a CI link — does not belong in that section at all**; those are groundwork, and putting them there is the tell that the real surface was never driven. A PR without that section is not merge-ready, and neither is one whose section is stale.
