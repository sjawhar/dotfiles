...
- **Vision (frame-reading):** sample every 2-3s plus densely at section boundaries; TRANSCRIBE what the screen says (hunt error text explicitly — frame statistics cannot read); legibility at half resolution (640x360); dead air, seams, truncation; anything sensitive on screen. **Small UI text (sidebar labels, form fields, terminal chrome) is never read from a full frame** — a full-frame vision read is untrustworthy in both directions, to pass and to fail: two reads of the same form frame returned `sonnet-3.5` vs `sonnet-5` and `gogs` vs `clu`, and only a crop of that region at native resolution or 2x transcribed stably. Crop before trusting any reading; two reads of one frame disagreeing is itself the signal to crop and re-read. (Measured by the Hosted Runs video review, 2026-09-12.)
