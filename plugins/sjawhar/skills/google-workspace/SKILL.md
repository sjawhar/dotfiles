...
| Create/suspend Workspace users or manage Google Group membership | `GWS_WORK_ADMIN_OAUTH` | human | **Not through `gws`.** `gws` 0.22.5 has no Admin SDK Directory alias (only `admin-reports`), and its documented `<api>:<version>` form for unlisted APIs is broken in this build: `gws admin:directory_v1 …` returns `validationError: Unknown service 'admin'` before auth is attempted (verified 2026-09-12, no credential involved). Use the credential directly under the same one-touch wrapper: `secrets GWS_WORK_ADMIN_OAUTH -- <script>` where the script exchanges the authorized-user JSON (`client_id`, `client_secret`, `refresh_token`) for an access token at `https://oauth2.googleapis.com/token` (`grant_type=refresh_token`) and calls `https://admin.googleapis.com/admin/directory/v1/…` with the bearer token. Never print the token or the JSON. Verified end to end (2026-09-13, 2sv-exempt cleanup): `GET /groups?customer=my_customer&maxResults=200` (find a group by email), `GET /groups/<groupKey>/members`, `GET /users/<email>?fields=primaryEmail,isEnrolledIn2Sv,isEnforcedIn2Sv,suspended,lastLoginTime`, the org-wide 2SV sweep `GET /users?customer=my_customer&query=isEnrolledIn2Sv%3Dfalse&maxResults=200`, and the group-membership REMOVE `DELETE /groups/<groupKey>/members/<memberEmail>` (returns 204, empty body). Gotcha: a members list read immediately after a DELETE can be stale for ~20 s — the 204 is the truth; re-read after a pause before concluding a removal failed. Also verified (2026-09-14, AGENTC-163 Slack-seat rename): membership-ADD `POST /groups/<groupKey>/members` with body `{"email": "<user>", "role": "MEMBER"}` (200, response `status: ACTIVE`); password reset `PUT /users/<key>` with `{"password": "<value>", "changePasswordAtNextLogin": false}` (200; generate the value in the script and never print it); `POST /users/<key>/signOut` (204, empty body; invalidates every session). Gotcha: a user in a 2SV-enforced OU who is not enrolled cannot sign in at all (`Your sign-in settings don't meet your organization's 2-Step Verification policy`); the sanctioned grace path is temporary membership in the hand-staffed `2sv-exempt@` group — `isEnforcedIn2Sv` on the user flips to false after ~20 min of propagation (observed 23:32Z add → 23:53Z effective), and removing the membership re-enforces immediately. Do a machine-mailbox sign-in only under that window, then remove the membership, rotate the password to an unrecorded value and `signOut`. Create and suspend writes (`POST /users`, `PUT /users/<key>` with `{"suspended": true}`) remain unexercised — the first session to run them records the exact calls here. The bundle is consented by `admin-sami@` (Sami's admin identity) since 2026-09-17, 8 scopes including `admin.directory.userschema.readonly`; a bundle minted by `sami@` has no admin authority. |
...
- **Never conclude a capability is missing because a name you guessed isn't there.** `gws` mirrors the Google APIs, so method and request names are Google's, not ones you'd invent — grepping for a plausible name and finding nothing tells you only that your guess was wrong. `gws schema <service.resource.method> --resolve-refs` prints every request type and field for that method; `gws --help` lists the services. Enumerate, then conclude.
- **Google Docs tabs are fully supported, under names you would not guess.** Create with a `docs.documents.batchUpdate` request keyed `addDocumentTab` (not `createTab`), whose `tabProperties` carry `title`, `iconEmoji`, and `parentTabId` for nesting; also `deleteTab` and `updateDocumentTabProperties`. Reading matters as much as writing: `docs.documents.get` omits tab content entirely unless you pass `includeTabsContent: true`, and requests target a specific tab via `TabsCriteria` / `tabId`.
