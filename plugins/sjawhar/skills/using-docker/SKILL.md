...
**`~/.docker/config.json` is Sami's file; no agent writes to it.** `docker login`, `crane auth login`, and `docker-credential-*` helpers all rewrite that file by default — three logins did on 2026-09-17 (two agents' `crane auth login`, one unintended), and every fresh `tl run` on the box then failed to pull `ghcr.io/trajectory-labs-pbc/agent-c` until he repaired it by hand. Sami, AGENTC-127 ask 740fe20e, 2026-09-17, verbatim: "I've fixed it, so STOP OVERWRITING MY GODDAMN DOCKER LOGIN!!!!!!!!!!" When a session needs a registry credential, it is the service token, scoped to the session's own process and its own config dir — `DOCKER_CONFIG=$(mktemp -d) secrets GHCR_PULL_TOKEN -- crane auth login ghcr.io -u x --password-stdin`, or `docker --config <tmpdir> login …` — never his login and never the shared file (verified: with `DOCKER_CONFIG` set, crane writes `<dir>/config.json` and `~/.docker/config.json`'s mtime does not move). If a pull fails with `denied` or `token does not match expected scopes`, the fix is your own scoped login, not a rewrite of his; a shared-file credential that stopped working is a finding to report, per the CLAUDE.md rule on personal credentials.
