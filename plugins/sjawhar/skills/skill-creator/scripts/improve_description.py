...
by calling the agent CLI as a subprocess. Supports both OpenCode and Claude Code.

For OpenCode: uses `opencode run --format json` and extracts text parts.
For Claude Code: uses `claude -p --output-format text` (same auth, no API key needed).
"""
...
from scripts.run_eval import detect_cli
...
def _call_opencode(prompt: str, model: str | None, timeout: int = 300) -> str:
    ...
    Parses the JSON event stream to extract text parts.
    """
    cmd = ["opencode", "run", "--format", "json", "--dir", "/tmp"]
    if model:
...
def _call_llm(
    prompt: str, model: str | None, timeout: int = 300, cli: str = "auto"
) -> str:
    """Call the appropriate CLI to get an LLM response."""
    if cli == "auto":
        cli = detect_cli()
    if cli == "opencode":
...
def improve_description(
    ...
    prompt = f"""You are optimizing a skill description for an AI coding agent skill called "{skill_name}". A "skill" is sort of like a prompt, but with progressive disclosure -- there's a title and description that the agent sees when deciding whether to use the skill, and then if it does use the skill, it reads the .md file which has lots more details and potentially links to other resources in the skill folder like helper files and scripts and additional documentation or examples.
...
Please respond with only the new description text in <new_description> tags, nothing else."""
    ...
    text = _call_llm(prompt, model, cli=cli)
    ...
    # Safety net: if the description is over 1024 chars, ask for a shorter rewrite
    if len(description) > 1024:
        ...
        shorten_text = _call_llm(shorten_prompt, model, cli=cli)
        match = re.search(
