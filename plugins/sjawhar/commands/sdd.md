...
**Smoke-test phase.** After implementation and before the final review pass, dispatch an agent to run the actual app, command, or endpoint and exercise every changed user workflow end to end. Sami's framing: everything is "useful functionality that you actually deliver and can prove works by actually running the app" — and that standard runs through planning and implementation too, not only at this one gate.
